...
function RideHistoryCard({
  ...
  return (
    <div
      ...
      data-ocid={`ride-history-${ride.id}`}
    >
      {/* Summary Row */}
      <button
        type="button"
      ...
      {expanded && (
        <div className="border-t border-border/40 px-4 pb-4 pt-3 space-y-3 bg-muted/20">
          ...
          {/* Driver placeholder info */}
          {ride.driverId && (
            <div className="flex items-center gap-3 bg-card rounded-xl p-3">
              <UserAvatar name="Driver" size="sm" />
              <div className="min-w-0">
                <p className="text-xs font-semibold truncate">
                  Verified Driver
                </p>
                <p className="text-[10px] text-muted-foreground">
                  ID: {ride.driverId.toText().slice(0, 12)}…
                </p>
              </div>
            </div>
          )}
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            title={
              activeFilter === "all"
                ? "No rides yet"
                : `No ${activeFilter === "women-only" ? "Women Only" : "General"} rides`
            }
            description={
              activeFilter === "all"
                ? "Book your first ride!"
                : "Try a different filter or book a new ride."
            }
...
export const Route = createRoute({
  ...
  component: PassengerHistoryPage,
});
